...
L.Control.BigImage = L.Control.extend({
  ...
  _createDownloadButton: function () {
    ...
    this._downloadBtn.addEventListener('click', () => {
      ...
      if (
        !scale_value ||
        scale_value < this.options.minScale ||
        scale_value > this.options.maxScale
      ) {
  ...
  _createCloseButton: function () {
    ...
    span.addEventListener('click', () => {
      ...
      this._containerParams.style.display = 'none';
      this._controlPanel.classList.remove(
        'polyline-measure-unicode-icon-disable',
      );
    });
  ...
  _loadTile: function (tilePoint, tilePos, layer, resolve) {
    ...
    image.onload = function () {
      if (!self.tilesImgs[imgIndex])
        self.tilesImgs[imgIndex] = { img: image, x: tilePos.x, y: tilePos.y };
      resolve();
    };
  ...
  _getMarkerLayer: function (layer, resolve) {
    ...
    pixelPoint = pixelPoint.subtract(
      new L.Point(self.bounds.min.x, self.bounds.min.y),
    );
  ...
  _getPathLayer: function (layer, resolve) {
    ...
    latlngs.forEach(latLng => {
      ...
      if (pixelPoint.x < self.canvas.width && pixelPoint.y < self.canvas.height)
        correct = 1;
    });
